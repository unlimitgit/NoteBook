...
public class MessageProcess
{
	...
	public static void searchPaneMouseMove(MouseEvent e) {
		int offset = GlobalVariables.searchPane.viewToModel(e.getPoint());			// Get mouse position
		try {
			GlobalVariables.messagePane.setText(null);								// Set message panel to display nothing
			int rowStart = Utilities.getRowStart(GlobalVariables.searchPane, offset); // Get the row of the mouse start position (related to string length)
			int textLength = GlobalVariables.searchPane.getDocument().getLength();	// Get the whole length of the text pane
			// If the rowStart is higher than textLength, it means the mouse is moving under the real end of text
			// Otherwise, the contents need be processed to get exact string of line
			if (textLength > rowStart) { 
				String dispContent1 =  GlobalVariables.searchPane.getText(0, rowStart);
				String dispContent2 =  GlobalVariables.searchPane.getText(rowStart, textLength-rowStart);
				String displayContent = null;
				int index1 = dispContent1.lastIndexOf("\n");
				int index2 = dispContent2.indexOf("\n");
				if ((index2 != -1) && (index1 != -1)){
